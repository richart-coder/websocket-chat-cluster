...
class RoomLockManager {
  ...
  async withLock(roomId, fn, timeout = 30000) {
    const releaseLock = await this.#acquireLock(roomId);
...
async function withRoomLock(roomId, fn, timeout = 30000) {
  return await roomLockManager.withLock(roomId, fn, timeout);
}
